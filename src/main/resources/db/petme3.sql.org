...
}
{
  ...
  (테이블명 설명) 
    ...
    장소-시간표사진 : 동물병원 가격표와 검사항목 등에 대한 이미지, 복수가 되나? = tb_place_id
    장소-방문자사진 : 동물병원에 방문해서 찍은 사진들에 대한 이미지들 = tb_place_id
    장소-메인이미지 : 사이트 자체에 대한 이미지들 = tb_place_id
    블로그-내부사진 : 사이트에 대한 블로그 이미지들 = tb_blog_id
    블로그-썸네일사진 : 블로그에 대한 썸네일 이미지, 복수가 되나? = tb_blog_id
    강사모-내부사진 : 강사모에 대한 썸네일 이미지 = tb_bulletin_board_id
    펫연구소-내부사진 : 펫연구소에 대한 컨텐츠 통 이미지 = tb_pet_raboratory_id
    펫연구소-썸네일사진 : 펫연구소에 대한 썸네일 이미지 = tb_pet_raboratory_id
    common_code로 만들어야 하는 부분이다.
  (DB 입력데이터 예제)  
    문자열 형태의 텍스트이다.
    "file|공지사항" "file|이벤트"
    "image|장소-시간표" "image|장소-방문자" "image|장소-메인" "image|블로그-메인" "image|블로그-썸네일" "image|강사모-메인" 
    "image|펫연구소-메인" "image|펫연구소-썸네일"

    "장소-시간표사진" "장소-방문자사진" "장소-메인이미지" "블로그이미지" "블로그썸네일" "강사모이미지" 
    "펫연구소-내부사진" "펫연구소-썸네일사진"
...
//------------------------------------------------------------
{
  ...
  CREATE TABLE `tb_crawling_naver_map` (
    ...
    `register_time` datetime DEFAULT now() NOT NULL COMMENT '정보 등록일시',
    PRIMARY KEY (`id`),
    KEY `index_tb_crawling_naver_map_place_name` (`place_name`)
...
//------------------------------------------------------------
{
  ...
  CREATE TABLE `tb_place` (
    ...
    `register_time` datetime DEFAULT now() NOT NULL COMMENT '정보 등록일시',
    PRIMARY KEY (`id`),
    KEY `index_tb_place_place_name` (`place_name`)
